...
# 使用支援中文的系统字体
font = pygame.font.SysFont("microsoftyaheiui", 28)  # Windows 微軟雅黑 # ❤️

# my_x, my_y = 100, 300
# enemy_x, enemy_y = 600, 300  # 對方的初始座標
my_x, my_y = 600, 300  # ❤️ 與 enemy 的初始位置對調，讓我們先放置在「對方」的位置
enemy_x, enemy_y = 100, 300   # ❤️ 與 my 的初始位置對調，讓我們先放置在「對方」的位置
my_hp = 10
enemy_hp = 10
...
# ==== 【TODO 1】網路 Socket 設定 ====
# 請在這裡建立 UDP Socket，綁定 5001 port 並設定為非阻塞 (Non-blocking) # ❤️
sock = socket.socket(socket.AF_INET, socket.SOCK_DGRAM) # AF_INET = IPv4, SOCK_DGRAM = UDP
sock.bind(('0.0.0.0', 5001)) # ❤️ 綁定到本機的 5001 port
...
while True:
    ...
    try:
        ...
        if package['type'] == 'pos':
            ...
            bullets.append({'x': package['x'], 'y': package['y'], 'dir': 10, 'owner': 'enemy'})  # ❤️
    ...
    for event in pygame.event.get():
        ...
        if event.type == pygame.KEYDOWN and event.key == pygame.K_SPACE:
            ...
            sock.sendto(json.dumps(shoot_package).encode(), (TARGET_IP, 5000)) # ❤️
    ...
    for b in bullets[:]:
        ...
        if b['owner'] == 'enemy' and my_rect.colliderect(bullet_rect):
            ...
            sock.sendto(json.dumps(hit_package).encode(), (TARGET_IP, 5000)) # ❤️
    ...
    # ==== 顯示 HP ====  # ❤️
    my_hp_text = font.render(f"玩家HP: {my_hp}", True, (59, 130, 246))  # ❤️
    enemy_hp_text = font.render(f"敵人HP: {enemy_hp}", True, (239, 68, 68))  # ❤️
    screen.blit(my_hp_text, (20, 20))  # ❤️
    screen.blit(enemy_hp_text, (650, 20))  # ❤️
    ...
    sock.sendto(json.dumps(pos_package).encode(), (TARGET_IP, 5000)) # ❤️